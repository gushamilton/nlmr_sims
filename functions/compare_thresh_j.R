compare_thresh_exponential_j <- function(exposure, outcome, seed = "123", reps = 10) {
  ...
  replicate_internally <- function(n_rep) {
    seed = seed + n_rep
    dat <-  sim_mydata_outcomes_thresh_j(n = 100000, seed = seed)
    dat_with_both <- generate_all_sumstats(data = dat, exposure = exposure, outcome = outcome, k = 100)
