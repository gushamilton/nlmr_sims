generate_outcome_plots_linear_replicates_RINT <- function(exposure, outcome, seed = "123", reps = 10) {
  
  
  replicate_internally <- function(n_rep) {
    seed = seed + n_rep
    dat <-  sim_mydata_outcomes_RINT(n = 100000, seed = seed)
  ...
  make_figures_replicates(d,exposure,outcome,dat,reps, "figures/post_rint/linear/", "_linear_exposure_RINT_") 
}
